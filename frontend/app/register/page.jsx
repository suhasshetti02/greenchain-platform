          ...
          <form className="space-y-5" onSubmit={handleSubmit}>
            ...
            {role === 'receiver' && (
              <div className="relative">
                 <label className="text-sm font-medium text-slate-700 block mb-1.5">Organization Address / Location</label>
                 <div className="relative group">
                    <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none text-slate-400 group-focus-within:text-[#0F766E] transition-colors">
                    ...
                    <input
                      type="text"
                      className="block w-full pl-10 pr-24 py-3 bg-white border border-slate-200 rounded-xl text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-[#0F766E]/20 focus:border-[#0F766E] transition-all duration-200 sm:text-sm shadow-sm"
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        setNotification({ type: 'success', message: "Detecting location..." });
                        ...
                        navigator.geolocation.getCurrentPosition(
                          async (position) => {
                             ...
                             // Optional: Reverse Geocode to fill address text
                             try {
                               ...
                               setNotification({ type: 'success', message: "Coordinates captured! Please add address details." });
                             }
                          },
                          (err) => {
                             console.error("Geolocation error", err);
                             setNotification({ type: 'error', message: "Unable to retrieve location." });
                          }
                        );
                      }}
                      className="absolute right-2 top-1.5 bottom-1.5 px-3 bg-teal-50 text-teal-700 text-xs font-bold rounded-lg hover:bg-teal-100 transition-colors flex items-center gap-1"
                    >
