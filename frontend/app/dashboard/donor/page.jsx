...
export default function DonorDashboardPage() {
  ...
  const handleConfirmPickup = async (donationId, confirmed) => {
    setProcessingId(donationId);
    try {
      // Optimistic Update
      setDonations(prev => {
        const newDonations = prev.map(d => {
          if (d.id === donationId) {
            const updatedClaims = (d.claims || []).map(c => {
              if (c.status === 'pending' || c.status === 'accepted') {
                // Backend now maps Confirmed -> 'accepted', No-Show -> 'cancelled'
                return { ...c, status: confirmed ? 'accepted' : 'cancelled' };
              }
              return c;
            });
            return {
              ...d,
              status: confirmed ? 'completed' : 'available',
      ...
      await api.donations.confirmPickup(donationId, confirmed);

      toast.success(confirmed ? "Pickup confirmed successfully" : "Marked as no-show");

      loadDashboard();
    } catch (err) {
      console.error("handleConfirmPickup Error:", err);
  ...
  const aiHint = useMemo(() => {
    ...
    if (upcoming.length > 0) {
      return {
        title: 'Expiring soon',
        body: `We found ${upcoming.length} donation${upcoming.length > 1 ? 's' : ''
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div>
              ...
              <p className="text-emerald-50 mt-2 max-w-lg text-lg">
                Your contributions are directly helping fight hunger and reduce environmental waste in your area.
              </p>
            </div>

            <div className="flex gap-8 divide-x divide-white/20">
              <div className="pl-4 first:pl-0">
                <p className="text-sm font-medium text-emerald-100 uppercase tracking-wide">Meals Served</p>
                <p className="text-4xl font-extrabold mt-1">
                  {Math.round((stats?.total_weight || donations.reduce((acc, d) => acc + (d.quantity_lbs || 0), 0)) * 0.83).toLocaleString()}
                </p>
              </div>
              <div className="pl-8">
                <p className="text-sm font-medium text-emerald-100 uppercase tracking-wide">CO₂ Saved (lbs)</p>
                <p className="text-4xl font-extrabold mt-1">
                  {Math.round((stats?.total_weight || donations.reduce((acc, d) => acc + (d.quantity_lbs || 0), 0)) * 2.5).toLocaleString()}
                </p>
              </div>
            </div>
          </div>
              ...
              {donations.map((donation) => (
                ...
                >
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    <StatusBadge
                      status={donation.status || 'operational'}
                    />
            ...
            <ul className="mt-4 space-y-4">
              {claimActivity.map((activity) => (
                <li key={activity.id} className="flex items-start gap-3">
                  <div className={`mt-1 h-2 w-2 rounded-full ${activity.status === 'accepted' || activity.status === 'completed' ? 'bg-slate-300' : 'bg-emerald-500'}`} />
                  <div className="flex-1">
                    ...
                    {/* Show actions only if pending. If 'accepted', it means donor confirmed pickup. */}
                    {(activity.status === 'pending') && (
                      <div className="mt-2 flex gap-2">
                        <Button
                          size="xs" // Assuming xs exists or small 
                          className="px-2 py-1 h-auto text-xs bg-emerald-600 hover:bg-emerald-700 disabled:opacity-50"
                          onClick={() => handleConfirmPickup(activity.donationId, true)}
                          disabled={processingId === activity.donationId}
                        >
                          {processingId === activity.donationId ? '...' : 'Confirm'}
                        </Button>
                        <Button
                          size="xs"
                          variant="outline"
                          className="px-2 py-1 h-auto text-xs text-rose-600 border-rose-200 hover:bg-rose-50 disabled:opacity-50"
                          onClick={() => handleConfirmPickup(activity.donationId, false)}
                          disabled={processingId === activity.donationId}
                        >
                          No-Show
                        </Button>
