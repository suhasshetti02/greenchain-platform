...
import { 
  Menu, X, ChevronRight, Leaf, Truck, Smartphone, 
    ...
    <div className="font-sans text-gray-800 bg-gray-50 overflow-x-hidden selection:bg-[#0F766E] selection:text-white">
      ...
      >
        <div className="container mx-auto px-4 md:px-8 flex justify-between items-center">
          {/* Logo */}
          <div className="flex items-center gap-2 font-bold text-2xl tracking-tight text-[#0F766E]">
            <div className="bg-[#D1FAE5] p-2 rounded-lg">
              <Leaf size={24} className="text-[#0F766E]" />
            </div>
            GreenChain
          </div>
          ...
          <div className="hidden md:flex items-center gap-8 font-medium text-gray-600">
            {navLinks.map((link) => (
              <a 
                key={link.name} 
                href={link.href} 
                className="hover:text-[#0F766E] transition-colors relative group"
              >
                {link.name}
                <span className="absolute -bottom-1 left-0 w-0 h-0.5 bg-[#0F766E] transition-all group-hover:w-full"></span>
              </a>
            ))}
          </div>
          ...
          <div className="flex items-center gap-4">
            <button 
              type="button"
              className="hidden md:flex items-center gap-2 bg-[#0F766E] text-white px-6 py-2.5 rounded-full hover:bg-[#0d645d] transition-all shadow-lg shadow-teal-700/20 hover:shadow-teal-700/40 transform hover:-translate-y-0.5 font-medium"
        ...
        {/* Mobile Menu */}
          ...
          <div className="absolute top-full left-0 w-full bg-white shadow-xl border-t md:hidden flex flex-col p-6 gap-4">
            ...
            <button 
              type="button"
              className="w-full bg-[#0F766E] text-white py-3 rounded-xl font-semibold mt-2"
              onClick={() => {
